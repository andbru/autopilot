...
struct filter updateCavallo(double ax, double ay, double az,
		  double wx, double wy, double wz,
		  double mx, double my, double mz, double dt) {
		  
	static bool firstTime = true;
	static double wDotState = 0;
	struct filter ret;
	...
	//  Filter wdot from wz
	double k = 0.95;
	double wdot = (wDotState + wz) * (1 - k) / dt;
	wDotState = k * wDotState + (1 -k) * wz;

	//  Generate sensor fusion result parameters
	ret.wdot = wdot * 180 / M_PI;													// Degrees per square second
	ret.w = wz * 180 / M_PI;															// Degrees per second
	ret.yaw = (180/M_PI)*atan2(2*(q0*q3+q1*q2), 1-2*(q2*q2+q3*q3));		// Degrees
